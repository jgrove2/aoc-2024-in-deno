...
function part2() {
  ...
  let splitWithPadding = [paddingLine, paddingLine, paddingLine].concat(split.map((x) => "..."+x+"..."))
  ...
  for (let i = 0; i < splitWithPadding.length; i++) {
    for (let j = 0; j < splitWithPadding[i].length; j++) {
      if (splitWithPadding[i][j] === "A") {
          if(!((splitWithPadding[i + 1][j+1] === "M" && splitWithPadding[i - 1][j - 1] === "S") || (splitWithPadding[i + 1][j+1] === "S" && splitWithPadding[i - 1][j - 1] === "M"))) { continue; }
          if(!((splitWithPadding[i - 1][j+1] === "M" && splitWithPadding[i + 1][j - 1] === "S") || (splitWithPadding[i - 1][j+1] === "S" && splitWithPadding[i + 1][j - 1] === "M"))) { continue; }
          finalTotal += 1
      }
    }
